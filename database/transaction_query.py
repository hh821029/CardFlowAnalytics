...
預設查詢預先建構之全維度 vw_rfm_analysis 視圖 (含商家正規化、支付管道與持卡對照)
"""
import os
import pandas as pd
import logging
...
def get_transactions(
    ...
    """
    通用交易資料讀取服務 (預設讀取已整合全維度資訊之 vw_rfm_analysis 視圖，相容 SQLite / PostgreSQL)
    """
    conditions = []
    params = {}
    
    # 統一 SQL 欄位映射與型態強轉 (SSOT 對齊，查詢預先建構好的 vw_rfm_analysis 視圖)
    query_parts = [
        "t.transaction_id",
    ...
    if not anchor_date and window != const.TimeWindow.LIFETIME:
        try:
            df_max = DBReader.read_sql("SELECT max(transaction_date) AS max_date FROM vw_rfm_analysis", db_path=db_path)
    ...
    sql = f"SELECT {', '.join(query_parts)} FROM vw_rfm_analysis t"
    if conditions:
        sql += " WHERE " + " AND ".join(conditions)
        
    try:
        df = DBReader.read_sql(sql, params=params, parse_dates=[const.COL_TXN_DATE], db_path=db_path)
        logger.info(f"📥 [DB 視圖提取 (vw_rfm_analysis)] 成功載入 {len(df)} 筆交易資料 (時間視窗: {window.name}, 基準日: {anchor_date})")
        return df
    except Exception as e:
        logger.warning(f"⚠️ 讀取 vw_rfm_analysis 視圖失敗 ({e})，降級嘗試從 all_transactions 原始表讀取...")
        try:
            fallback_parts = [
                "transaction_id",
                f"transaction_date AS {const.COL_TXN_DATE}",
                f"merchant_name AS {const.COL_MERCHANT_DISPLAY}",
                f"merchant_name AS {const.COL_NORMALIZED_MERCHANT}",
                "'' AS ec_platform",
                "'' AS payment_process",
...
def query_transactions_modular(
    ...
    """
    動態 SQL 條件查詢服務 (直接查詢預先構建好之 vw_rfm_analysis 視圖，相容 SQLite / PostgreSQL)
    """
    conditions = []
    params = {}
    ...
    try:
        df = DBReader.read_sql(sql, params=params, parse_dates=[const.COL_TXN_DATE], db_path=db_path)
        logger.info(f"📥 [DB 條件篩選 (vw_rfm_analysis)] 成功載入 {len(df)} 筆交易資料 (條件數: {len(conditions)})")
        return df
    except Exception as e:
        logger.warning(f"⚠️ 讀取 vw_rfm_analysis 視圖條件查詢失敗 ({e})，降級嘗試從 all_transactions 原始表讀取...")
        try:
            fallback_parts = [
                "transaction_id",
                f"transaction_date AS {const.COL_TXN_DATE}",
